...
function default_1(_a) {
    ...
    function insertHelper(programPath, value) {
        var destinationPath = programPath.get("body").find(function (path) { return !path.node._isHelperDefinition && !path.isImportDeclaration(); });
        if (destinationPath.isVariableDeclaration()) {
            var before = destinationPath.get("declarations").filter(function (path) { return path.node._isHelperDefinition; });
            var after = destinationPath.get("declarations").filter(function (path) { return !path.node._isHelperDefinition; });
